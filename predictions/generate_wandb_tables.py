...
class ModelNameProcessor:
    ...
    def process_model_name(self, name: str, for_latex: bool = True) -> str:
        return self.format_for_latex(name) if for_latex else self.normalize_base_name(name)
...
def main():
                        ...
                        if "exact_match" in metrics and "f1" in metrics:
                            ...
                            for metric_name, value in metrics.items():
                                if isinstance(value, (float, int)):
                                    ...
                                    results_by_model[model_display_name][key] = float(value) * 100.0
                                    if key not in all_columns:
                                        all_columns.append(key)

    print(set(llms["all"] + small_llm["all"] + small_llm_2["all"] + private_llm["all"]).difference(set(all_model_names)))
    df = pd.DataFrame.from_dict(results_by_model, orient="index", columns=all_columns)
    df["Composite Score"] = df.mean(axis=1)
    df.index.name = "model_name"
    df.reset_index(inplace=True)
    df["model_name"] = df["model_name"].apply(lambda x: processor.process_model_name(x, for_latex=True))
    ...
    df_sorted = df[["model_name", "Composite Score"]].sort_values("Composite Score", ascending=False).reset_index(drop=True)
    top_n = 48
    ...
    df_2.to_latex(FULL_TABLE_LATEX_short, index=False, float_format="%.2f", escape=False)
