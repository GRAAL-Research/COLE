...
class HFModel(Model):
    ...
    def __init__(self, model_name, task="text-generation", token=None, lazy_load=True, batch_size=8):
...
class HFLLMModel(HFModel):
    ...
    def infer(self, prompts: str, conditions=None):
        ...
        for sub_batch in chunk_list(prompts, self.batch_size):
            try:
                batch_outputs = self.pipe(sub_batch)
            except Exception as e:
                print(f"️ Échec inference batch {sub_batch[:2]} : {e}")
            ...
            for single_output in batch_outputs:
                if isinstance(single_output, list) and len(single_output) > 0:
                    ...
                    text = single_output.get("generated_text", "") if isinstance(single_output, dict) else ""
