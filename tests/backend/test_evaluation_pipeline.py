...
class EValuationPipelineQFRCoLATest(TestCase):
    def assertDictIsEmpty(self, dict_to_assert):
        self.assertDictEqual(dict_to_assert, {})

    def setUp(self):
        # We create a fake submission
        self.a_task = "qfrcola"
        self.a_prediction_perfect_score = [1, 1, 0, 0, 1]
        max_samples = 5  # i.e. len of prediction space
        self.a_prediction_perfect_score = {
            f"custom|{self.a_task}|0|0": self.a_prediction_perfect_score
        }

        self.a_prediction_imperfect_score = [0, 0, 1, 1, 0]
        self.a_prediction_imperfect_score = {
            f"custom|{self.a_task}|0|0": self.a_prediction_perfect_score
        }

        # We extract the task prediction dictionary
        ...
        # and create the task str
        # We can any of the two imperfect or perfect score
...
class EValuationPipelineQFRBLiMPTest(TestCase):
    def assertDictIsEmpty(self, dict_to_assert):
        self.assertDictEqual(dict_to_assert, {})

    def setUp(self):
        # We create a fake submission
        self.a_task = "qfrblimp"
        self.a_prediction_perfect_score = [0, 0, 0, 0, 0]
        max_samples = 5  # i.e. len of prediction space
        self.a_prediction_perfect_score = {
            f"custom|{self.a_task}|0|0": self.a_prediction_perfect_score
        }

        self.a_prediction_imperfect_score = [1, 1, 1, 1, 1]
        self.a_prediction_imperfect_score = {
            f"custom|{self.a_task}|0|0": self.a_prediction_perfect_score
        }
